...
export function RoleCatalogDialog({ open, onOpenChange }: RoleCatalogDialogProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={(v) => { if (!v) resetForm(); onOpenChange(v); }}>
            <DialogContent className="sm:max-w-[800px] border-zinc-800 bg-zinc-900 text-white rounded-sm h-[80vh] flex flex-col p-0 gap-0">
                ...
                <div className="flex-1 flex overflow-hidden">
                    {/* List Section */}
                    <div className="w-1/3 border-r border-zinc-800 overflow-y-auto p-4 space-y-2 bg-zinc-950/30">
                        <h3 className="text-xs font-mono font-bold text-zinc-500 uppercase mb-3">Defined Roles</h3>
                        {isLoading ? (
                            ...
                            roles.map(role => (
                                <div
                                    key={role.id}
                                    className={`p-3 rounded-sm border cursor-pointer hover:bg-zinc-800 transition-colors ${editingRole?.id === role.id ? 'bg-zinc-800 border-primary/50' : 'bg-zinc-950 border-zinc-800'}`}
                                    onClick={() => handleEdit(role)}
                                >
                                    <div className="flex justify-between items-start mb-1">
                                        <span className="font-semibold text-sm text-white">{role.roleName}</span>
                                        <Badge variant="outline" className="text-[10px] h-4 border-zinc-700 text-zinc-400">{role.department}</Badge>
                                    </div>
                        ...
                        <Button variant="outline" className="w-full border-dashed border-zinc-700 text-zinc-400 hover:text-white" onClick={resetForm}>
                    ...
                    <div className="w-2/3 p-6 overflow-y-auto">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="space-y-2 w-1/2 pr-2">
                                <Label className="text-zinc-400">Default Billable Rate ($/hr)</Label>
                                <Input
                                    type="number"
                                    value={formData.defaultBillableRate}
                                    onChange={e => setFormData({ ...formData, defaultBillableRate: e.target.value })}
                                    className="bg-zinc-950 border-zinc-800 text-green-500 font-mono"
                                />
                            </div>

                            <div className="space-y-3 pt-2">
                                <Label className="text-zinc-400">Allowed Activities / Skills</Label>
                                <div className="flex gap-2">
                                    ...
                                    />
                                    <Button type="button" onClick={handleAddActivity} variant="secondary" className="border border-zinc-700 bg-zinc-800"><Plus className="size-4" /></Button>
                                </div>
                                <div className="flex flex-wrap gap-2 p-3 bg-zinc-950/50 rounded-sm border border-zinc-800 min-h-[60px]">
                                    {currentActivities.length === 0 && <span className="text-xs text-zinc-600 italic">No activities defined</span>}
                                    {currentActivities.map(activity => (
                                        <Badge key={activity} className="bg-zinc-800 text-zinc-300 hover:bg-zinc-700 cursor-pointer pr-1" onClick={() => removeActivity(activity)}>
                                            {activity} <span className="ml-1 text-zinc-500 hover:text-red-400">×</span>
                                        </Badge>
                                    ))}
                                </div>
                            </div>
